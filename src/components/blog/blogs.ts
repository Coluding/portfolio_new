...
export const blogs: BlogTemplateProps[] = [
    {
        ...
        \begin{equation} $\mathbf{S}_i^\mathrm{w}(Q_i,K_{S_i}) = softmax({Q_i}{K_{S_i}}^\mathrm{T})$ \end{equation}
...
To ease the understanding in the following section, a set $H$ is defined which contains all the additional hyperparameters that have to be considered using the novel approaches. Currently $H$ can be described as: $H = \{W\}$, 
